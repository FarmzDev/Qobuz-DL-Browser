...
class QobuzDL {
        ...
        return parser.parseFromString(`
            <div class="w-full flex items-center justify-start transition-all p-1 text-nowrap hover:bg-zinc-900/30 px-3">
                <div class="flex flex-col justify-center items-start w-full overflow-hidden">
                    <p class="font-medium truncate w-full">${this.formatTrackTitle(track)}</p>
    ...
    createDownloadJob(result, type) {
        ...
        const element = parser.parseFromString(`
                <div data-id="${result.id}" class="relative w-full bg-tertiary p-3 overflow-hidden rounded-[24px] flex flex-col justify-center gap-2 flex-shrink-0" data-state="incomplete">
                    ...
                    <div class="w-full overflow-hidden flex items-start gap-2 px-2 z-[51]">
                        <img src="${type.toLowerCase() === "track" ? result.album.image.small : result.image.small}" alt="" class="w-[100px] h-[100px]">
                        <div class="flex flex-col w-full overflow-hidden">
                            <h2 class="text-nowrap text-xl md:text-xl font-medium truncate pr-3">${type.toLowerCase() === "track" ? `${result.title}${result.version ? " (" + result.version + ")" : ""}` : result.title}</h2>
                    ...
                    <div class="w-full flex flex-col items-center justify-center px-2 gap-1 z-[51]">
                        <progress-bar class="w-full bg-zinc-500 h-1.5 rounded-full overflow-hidden">
                            <div class="h-full transition-all" style="width: 50%;"></div>
                        </progress-bar>
                        <p class="text-white text-center" data-status>Downloading...</p>
                    </div>
                </div>
            `, "text/html").body.firstChild;
        ...
        button.addEventListener("click", async () => {
            try {
                ...
                const notification = this.notyf.success({
                    message: `Added "${this.formatTrackTitle(track)}" to the download queue.`,
                    duration: 3000,
                    dismissible: true,
                    ripple: true,
                  });
                ...
                downloadJob.status.innerHTML = `<a class="hover:underline" href="${downloadBlobLink}" download="${(track.artists.length > 0 ? track.artists.map(artist => artist.name).join(", ") : "Various Artists") + " - " + this.formatTrackTitle(track) + ".flac"}">Track not downloaded to your device? Click here to download it again.</a>`
                ...
                downloadAlbumButton.addEventListener("click", async () => {
                    try {
                        ...
                        for (const [index, track] of album.tracks.items.entries()) {
                            ...
                            if (totalAlbumBytes >= (2 * 1024 * 1024 * 1024)) {
                                downloadJob["progress-bar"].classList.remove("bg-amber-500");
                                downloadJob["progress-bar"].classList.add("bg-red-500");
                                downloadJob["progress-bar"].classList.remove("transition-all");
                                downloadJob["progress-bar"].style.width = "100%"
                                downloadJob.status.textContent = "Album exceeds browser memory limit.";
                                downloadJob.base.setAttribute("data-state", "complete");
                                this.showError("Album cannot be downloaded as a ZIP. It exceeds the browser memory limit (2GB). Try downloading tracks individually instead.");
                                return
                            }
                        }
                        downloadJob["progress-bar"].classList.remove("bg-amber-500");
                        ...
                        for (const [index, track] of album.tracks.items.entries()) {
                            ...
                            while (true) {
                                ...
                                downloadJob.status.textContent = `Downloading "${this.formatTrackTitle(track)}"... (${this.formatBytes(downloadedBytes)} / ${this.formatBytes(totalAlbumBytes)})`
                            ...
                            downloadJob.status.textContent = `Applying Metadata to "${this.formatTrackTitle(track)}"... (${this.formatBytes(downloadedBytes)} / ${this.formatBytes(totalAlbumBytes)})`;
                        ...
                        var zip = new JSZip();
                        for (const [index, track] of album.tracks.items.entries()) {
                            zip.file(this.cleanFileName(`${(index+1).toString().padStart(2, '0')} ${this.formatTrackTitle(track)}.flac`), tracks[index]);
                        }
                        downloadBlobLink = URL.createObjectURL(await zip.generateAsync({type:"blob"}, (update) => {
                            downloadJob["progress-bar"].classList.add("transition-all");
                        ...
                        downloadJob.status.innerHTML = `<a class="hover:underline" href="${downloadBlobLink}" download="${(album.artists.length > 0 ? album.artists.map(artist => artist.name).join(", ") : "Various Artists") + " - " + album.title + ".zip"}">Track not downloaded to your device? Click here to download it again.</a>`
                        downloadJob.status.querySelector("a").click();
                    } catch (e) {
                        if (!(e.name === 'AbortError')) {
                            downloadAlbumButton.removeAttribute("disabled");
    ...
    async applyMetadata(trackBlob, resultData) {
        ...
        const albumArt = await (await fetch(resultData.album.image.large)).arrayBuffer();
        ffmpeg.FS("writeFile", "albumArt.jpg", new Uint8Array(albumArt));
        await ffmpeg.run(
            "-i", "input.flac",
            "-i", "metadata.txt",
            "-map_metadata", "1",
